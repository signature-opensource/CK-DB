...
public class MCResStringTests
{
    ...
    public void fallbaks_between_french_and_english_and_german_cultures()
    {
        ...
        using( var ctx = new SqlStandardCallContext() )
        {
            ...
            p.MCResStringTable.SetString( ctx, deId, DeCultureId, "Nur in deutscher Sprache." );
            p.MCResStringTable.SetString( ctx, allId, FrCultureId, "Français (et Anglais et Allemand)." );
            p.MCResStringTable.SetString( ctx, allId, EnCultureId, "English (and French and German)." );
            p.MCResStringTable.SetString( ctx, allId, DeCultureId, "Deutsch (und Englisch und Französisch)." );
    ...
    static void AssumeFallbackTestEnglishAndFrenchResources( Package p, SqlStandardCallContext ctx, out int noValuesId, out int enId, out int frId, out int bothId )
    {
        ...
        p.MCResStringTable.SetString( ctx, enId, EnCultureId, "Only in English." );
        frId = p.ResTable.Create( ctx );
        p.MCResStringTable.SetString( ctx, frId, FrCultureId, "Seulement en Français." );
        bothId = p.ResTable.Create( ctx );
        p.MCResStringTable.SetString( ctx, bothId, EnCultureId, "English (and French)." );
        p.MCResStringTable.SetString( ctx, bothId, FrCultureId, "Français (et Anglais)." );
    }
    ...
    {
        p.Database.ExecuteScalar( "select Value from CK.vMCResString where ResId=@0 and CultureId = @1", resId, cultureId )
                .ShouldBe( expectedValue );
        p.Database.ExecuteScalar( "select MatchedCultureId from CK.vMCResString where ResId=@0 and CultureId = @1", resId, cultureId )
    ...
    public void setting_and_clearing_values_traverses_culture_hierarchy()
    {
        ...
        using( var ctx = new SqlStandardCallContext() )
        {
            ...
            p.MCResStringTable.SetString( ctx, resId, EnCultureId, "English root." );
            ...
            p.MCResStringTable.SetString( ctx, resId, FrCultureId, "Français." );
            CheckString( p, resId, EnCultureId, "English root.", EnCultureId );
            ...
            p.MCResStringTable.SetString( ctx, resId, FrCaCultureId, "Québécois." );
            CheckString( p, resId, EnCultureId, "English root.", EnCultureId );
            CheckString( p, resId, FrCultureId, "Français.", FrCultureId );
            ...
            p.MCResStringTable.SetString( ctx, resId, DeCultureId, "Deutsch." );
            CheckString( p, resId, FrCultureId, "Français.", FrCultureId );
            CheckString( p, resId, FrCaCultureId, "Québécois.", FrCaCultureId );
            ...
            p.MCResStringTable.SetString( ctx, resId, FrCaCultureId, null );
            CheckString( p, resId, FrCaCultureId, "Français.", FrCultureId );

            // Clearing the mid level cascades fr-CA back to the root.
            p.MCResStringTable.SetString( ctx, resId, FrCultureId, null );
            CheckString( p, resId, FrCultureId, "English root.", EnCultureId );
            CheckString( p, resId, FrCaCultureId, "English root.", EnCultureId );
    ...
    public void english_is_the_ultimate_fallback_for_cultures_not_linked_to_english()
    {
        ...
        using( var ctx = new SqlStandardCallContext() )
        {
            ...
            p.MCResStringTable.SetString( ctx, resId, EnCultureId, "English value." );
            ...
            p.MCResStringTable.SetString( ctx, resId, OrphanCultureId, "Orphan value." );
            ...
            p.MCResStringTable.SetString( ctx, resId, OrphanCultureId, null );
            CheckString( p, resId, OrphanCultureId, "English value.", EnCultureId );
    ...
    public void value_in_any_language_is_returned_when_neither_culture_chain_nor_english_have_a_value()
    {
        ...
        using( var ctx = new SqlStandardCallContext() )
        {
            ...
            p.MCResStringTable.SetString( ctx, resId, OrphanCultureId2, "Hola" );
            ...
            p.MCResStringTable.SetString( ctx, resId, EnCultureId, "English." );
    ...
    public void destroying_the_resource_destroys_the_string_values()
    {
        ...
        using( var ctx = new SqlStandardCallContext() )
        {
            ...
            p.Database.ExecuteReader( "select Value from CK.vMCResString where ResId=@0", bothId )
                .Rows.ShouldBeEmpty();

            Assert.DoesNotThrow( () => p.MCResStringTable.SetString( ctx, bothId, EnCultureId, null ) );
        }
